...
class DistBuilder:
    ...
    @staticmethod
    def _run_py_install(version: str, arch: str):
        install_arch: str = f'-{arch}'
        if arch == 'win32':
            install_arch = '-32'
        elif arch == 'amd64':
            install_arch = '-64'
        subprocess.check_output(['py', 'install', f'{version}{install_arch}'], text=True)
    ...
    def install_requirements(version: str, target_dir: str, requirements_file: str, arch: str):
        ...
        # needed to ensure the requested arch of the requested version of python is installed.
        DistBuilder._run_py_install(version, arch)
        _arch = f'win_{arch}'
        if arch == 'win32':
            _arch = 'win32'
        ...
        subprocess.check_output([
            ...
            '--platform', _arch, '--only-binary=:all:'], text=True)
        subprocess.check_output([
            'git', 'clone', 'https://github.com/AraHaan/_memimporter.git',
            os.path.join(os.path.join(target_dir, 'pkg-build'), '_memimporter')
        ])
        major, minor, patch = version.split(".")
        subprocess.check_output([
            'msbuild',
            os.path.join(
                os.path.join(os.path.join(os.path.join(target_dir, 'pkg-build'), '_memimporter'), 'pcbuild'),
                'pcbuild.sln'),
            '/p:Configuration=Release', f'/p:Platform={build_arch}',
            f"/p:OutDir={os.path.join(target_dir, 'build_tmp')}",
            f'/p:PythonMajorVersion={major}', f'/p:PythonMinorVersion={minor}', f'/p:PythonPatchVersion={patch}'
        ])

        # cleanup the clone of the _memimporter repository.
        shutil.rmtree(os.path.join(target_dir, 'pkg-build'), ignore_errors=True)
