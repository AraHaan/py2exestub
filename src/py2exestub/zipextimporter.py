    ...
    for suffix, is_ext, is_package in searchorder:
        ...
        if hasattr(zipimporter, '_files'):
            if path not in self._files:
                continue
        elif hasattr(zipimporter, '_get_files'):
            if path not in self._get_files():
                continue
        if not is_ext:
...
# Return the path if it represents a directory.
...
class ZipExtensionImporter(zipimporter):
    ...
    def __init__(self, path_or_importer):
        if isinstance(path_or_importer, zipimporter):
            ...
            if hasattr(zipimporter, '_files') or hasattr(zipimporter, '_get_files'):  # py <= 37, built-in
...
def install(hook=True):
    '''Install the zipextimporter.'''
    # _files was changed to a _get_files method in Python 3.13 and newer.
    if not hasattr(zipimporter, '_files') and not hasattr(zipimporter, '_get_files'):
        hook = False
    if hook:
        _install_hook()
    else:
